	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ac.equals("login")) {
			...
			if (list.size() == 1) {
				...
				gor("/zxnxptjshop/admin/index.jsp", request, response);
			} else {
		...
		if (ac.equals("ddfh")) {
			...
			ArrayList<HashMap> dddetaillist = (ArrayList<HashMap>) dao
					.select("select * from dddetail where ddno="
							+ ddmap.get("ddno"));
			boolean flag = true;// 用作订单葡萄酒库存校验结果
			for (HashMap dddetailmap : dddetaillist) {
				// 如果其中某个葡萄酒的数量大于其库存量 则置 FLASE标识
			...
			if (flag) {
				...
				request.setAttribute("info", "订单中葡萄酒库存不足，发货失败!");
				go("/admin/ddgl.jsp", request, response);
			}
		}
		...
		if (ac.equals("pj")) {
			...
			dao.commOper("insert into pj (goodid,goodsaver,memberid,jb,msg,savetime,hf) values "
					...
					+ savetime
					+ "','') ");
			request.setAttribute("suc", "葡萄酒评价成功!");
			go("/mydd.jsp", request, response);
		}
		// 管理员回复评价
		if (ac.equals("pjhf")) {
		...
		if (ac.equals("addfav")) {
			...
			if (member == null) {
				...
				if (cklist.size() == 0) {
					...
					request.setAttribute("suc", "此葡萄酒已收藏过!");
					go("/" + page, request, response);
				}
			}
		}
